...
//    val conf = new SparkConf().setAppName("AggregateSum Application").setMaster("local")
    val conf = new SparkConf().setAppName("AggregateSum Application")
    ...
    val c = sum_int.count()
    println(s"Query Finished: Aggregate sum - there are $c lines here.")
