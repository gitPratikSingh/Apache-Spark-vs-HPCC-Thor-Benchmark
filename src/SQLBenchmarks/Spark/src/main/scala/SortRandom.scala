...
//    val conf = new SparkConf().setAppName("SparkBenchmark Application").setMaster("local")
    val conf = new SparkConf().setAppName("SparkBenchmark Application")
    ...
    println(s"Query Finished: Sort random - there are $c lines here.")
