...
object AggregateCountTotal {
  def main(args: Array[String]) {
//    val conf = new SparkConf().setAppName("AggregateCountTotal Application").setMaster("local")
    val conf = new SparkConf().setAppName("AggregateCountTotal Application")
    ...
    //Count total
    val count_total = time{sqlContext.sql("select count(_c0) from wutable")}
    val c = count_total.count()
    println(s"Query Finished: Count total - there are $c lines here.")
